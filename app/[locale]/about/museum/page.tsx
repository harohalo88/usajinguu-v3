...
const HEADER_IMG = 'https://images.unsplash.com/photo-1772183095415-88a2c5135bc9?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1920';
  ...
  return (
    <div className="w-full min-h-screen" style={{ backgroundColor: C.ivory }}>
      ...
      <section style={{ position: 'relative', height: '280px', overflow: 'hidden' }}>
        <img src={HEADER_IMG} alt="宝物館のご案内" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 40%' }} />
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center" style={{ zIndex: 10, paddingTop: '72px' }}>
          ...
          <nav className="flex items-center gap-2 mb-6">
            <Link href={`/${locale}`} style={{ fontFamily: 'var(--font-sans)', color: 'rgba(250,248,245,0.55)', fontSize: '0.62rem', letterSpacing: '0.1em', textDecoration: 'none' }}>ホーム</Link>
            <ChevronRight size={11} style={{ color: 'rgba(250,248,245,0.3)' }} />
            <span style={{ fontFamily: 'var(--font-sans)', color: 'rgba(250,248,245,0.55)', fontSize: '0.62rem', letterSpacing: '0.1em' }}>宇佐神宮について</span>
            <ChevronRight size={11} style={{ color: 'rgba(250,248,245,0.3)' }} />
            <span style={{ fontFamily: 'var(--font-sans)', color: 'rgba(196,154,58,0.85)', fontSize: '0.62rem', letterSpacing: '0.1em' }}>宝物館のご案内</span>
          </nav>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="text-center">
            ...
            <h1 style={{ fontFamily: 'var(--font-serif)', color: '#faf8f5', fontSize: 'clamp(1.6rem, 4vw, 2.4rem)', fontWeight: 300, letterSpacing: '0.25em' }}>宝物館のご案内</h1>
        ...
        <div className="max-w-3xl mx-auto px-6 md:px-10 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }}>
            ...
            <div style={{ padding: '48px 40px', border: `1px solid ${C.border}`, backgroundColor: C.stone }}>
              {/* Kanji decoration */}
              <div style={{ fontFamily: 'var(--font-serif)', color: 'rgba(165,0,0,0.06)', fontSize: '8rem', lineHeight: 1, marginBottom: '-1.5rem', userSelect: 'none', overflow: 'hidden', position: 'relative' }}>
                準備
              </div>
              <div style={{ position: 'relative', zIndex: 1 }}>
                <h2 style={{ fontFamily: 'var(--font-serif)', color: C.crimson, fontSize: '1.3rem', letterSpacing: '0.25em', fontWeight: 400, marginBottom: '16px' }}>準備中</h2>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-10">
              <Link href={`/${locale}`} style={{ fontFamily: 'var(--font-sans)', fontSize: '0.72rem', letterSpacing: '0.15em', color: '#faf8f5', backgroundColor: C.crimson, padding: '10px 24px', textDecoration: 'none', display: 'inline-block' }}>
                ホームへ戻る
              </Link>
            </div>
            ...
            <div style={{ marginTop: '48px', paddingTop: '32px', borderTop: `1px solid ${C.border}` }}>
              <p style={{ fontFamily: 'var(--font-sans)', color: C.textMute, fontSize: '0.75rem', letterSpacing: '0.08em', marginBottom: '12px' }}>
                お問い合わせはこちらから
              </p>
